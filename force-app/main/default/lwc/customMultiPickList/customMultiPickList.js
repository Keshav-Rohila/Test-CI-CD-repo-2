...
export default class CustomMultiPickList extends LightningElement {
    ...
    renderedCallback() {
        if(!this.init) {
            this.options = JSON.parse(JSON.stringify(this.pickListOptions));
    ...
    resetSelection(){
        ...
        const tempRequired = this.required;
        this.required = false;
        this.postSelect();
    ...
    dispatchUpdatedSelection(){
        this.dispatchEvent(new CustomEvent('selection', {detail: {options: this.options, label: this.label}}));
    }
